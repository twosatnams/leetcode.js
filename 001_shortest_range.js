...
function shortestRange(lists, k) {
	...
	var minList = findMinList(lists);
	var maxList = findMaxList(lists);

	var mininum = minList.shift();
	var maximum = maxList.shift();
	while (listsEmpty === false) {
		if (rangeSize < maximum - mininum) {
			rangeSize = maximum - mininum;
		}

		mininum = minList.shift();

	}
}

function listsEmpty(lists) {
	for (var i = 0; i < lists.length; i++) {
		if (lists[i].length === 0) return true;
	}
	return false;
}
